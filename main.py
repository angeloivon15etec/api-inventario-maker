...
# 2. O Modelo Pydantic (Atualizado com o campo obrigatório)
class ComponenteSchema(BaseModel):
    ...
    estado_conservacao: str = Field(..., description="Estado de conservação do componente") # <-- FALTAVA ESTE CAMPO [cite: 6, 7]
...
estoque_laboratorio = [
    ...
    {"id": 3, "nome": "Esteira em Acrílico", "quantidade": 2, "categoria": "Mecânica", "estado_conservacao": "Excelente"}
]
...
def atualizar_componente(componente_id: int, dados_atualizados: ComponenteSchema):
    for item in estoque_laboratorio:
        if item["id"] == componente_id:
            ...
            item["estado_conservacao"] = dados_atualizados.estado_conservacao # <-- ADICIONADO PARA ATUALIZAR O CAMPO 
